...
(function() {
  var Morocco, buildMatchers, commander, configure, defaults, document, format, fs, getLanguage, highlightjs, languages, marked, parse, path, run, trim, version, write, _,
    __slice = [].slice;
  ...
  document = function(options, callback) {
    ...
    config.onNewSection = function(section) {
      var docLines, example, examples, i, input, line, output, _i, _len;
      examples = null;
      docLines = section.docsText.split('\n');
      for (i = _i = 0, _len = docLines.length; _i < _len; i = ++_i) {
        line = docLines[i];
      ...
      if (examples != null) {
        ...
        return section.examples = (function() {
          ...
          for (_j = 0, _len1 = examples.length; _j < _len1; _j++) {
            ...
            _ref = example.split('// =>', 2), input = _ref[0], output = _ref[1];
            _ref1 = [trim(input), trim(output)], input = _ref1[0], output = _ref1[1];
            _results.push({
              input: input,
              output: output
            });
  ...
  parse = function(source, code, config) {
    var codeText, docsText, handleNewSection, hasCode, i, isText, lang, line, lines, match, maybeCode, save, sections, _i, _j, _len, _len1;
    if (config == null) {
      config = {};
    }
    lines = code.split('\n');
    ...
    handleNewSection = config.onNewSection || function(section) {};
    save = function() {
      var section;
      section = {
        docsText: docsText,
  ...
  configure = function(options) {
    ...
    config.sources = options.args.filter(function(source) {
      ...
      lang = getLanguage(source, config);
      if (!lang) {
        console.warn("morocco: skipped unknown type (" + (path.basename(source)) + ")");
      }
      return lang;
    }).sort();
    return config;
  ...
  run = function(args) {
    ...
    c = defaults;
    commander.version(version).usage('[options] files').option('-L, --languages [file]', 'use a custom languages.json', _.compose(JSON.parse, fs.readFileSync)).option('-o, --output [path]', 'output to a given folder', c.output).option('-c, --css [file]', 'use a custom css file', c.css).option('-e, --extension [ext]', 'assume a file extension for all inputs', c.extension).parse(args).name = "morocco";
